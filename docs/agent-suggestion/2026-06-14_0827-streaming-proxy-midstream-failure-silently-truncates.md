...
- **APPLIED 2026-06-15:** Set to Taken after verifying the proxy now catches `httpx.HTTPError` raised mid-stream, emits an in-band `vampire_upstream_error` SSE frame plus `data: [DONE]`, and includes regression coverage for upstream streaming aborts.
