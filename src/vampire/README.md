...
The runnable core of **LLM Vampire**: a FastAPI gateway that aggregates
owner-approved local LLM services into one governed, private AI service behind
a single OpenAI-compatible URL. The package is built in phases per
...
| [`config.py`](config.py) | Runtime settings. Vampire listens on port 7777 and uses a provider-neutral fallback service; everything is overridable via `VAMPIRE_*` environment variables |
| [`models.py`](models.py) | Core Pydantic orchestration models (DESIGN-API.md §4) |
| [`proxy.py`](proxy.py) | Transparent proxy: forwards `/v1/*` to a selected provider, preserving streaming responses and OpenAI-style upstream errors |
| [`providers/`](providers) | Provider adapters that normalize OpenAI-compatible and Ollama model inventories |
| [`registry.py`](registry.py) | In-memory registry of approved local LLM service nodes |
| [`cluster.py`](cluster.py) | Node health checks, local/static/dev-subnet discovery, model inventory aggregation, metrics, and SSRF target validation |
