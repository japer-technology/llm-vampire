...
| macOS | `.icns` | Apple icon set referenced by `packaging/macos/LLMVampire.spec` (currently `icon=None`; wire the `.icns` here when finalized). |
...
- **macOS** — set `icon="../common/icons/<name>.icns"` in the `BUNDLE(...)` call
  of `packaging/macos/LLMVampire.spec`.
- **Windows** — set `icon="..\\common\\icons\\<name>.ico"` in the `EXE(...)` call
  of `packaging/windows/LLMVampire.spec`, and/or reference it from
  `packaging/windows/installer.iss`.
- **Ubuntu** — install the `.png` sizes and point `Icon=` in
  `packaging/ubuntu/llm-vampire.desktop` at the installed icon name.
